import { ref } from 'vue'

let commentList = ref([
  //国家数字安全战略
  {
    topicId: '0',
    btnShow: false,
    index: '0',
    replyComment: '',
    myName: 'Lana Del Rey',
    myHeader:
      'https://ae01.alicdn.com/kf/Hd60a3f7c06fd47ae85624badd32ce54dv.jpg',
    myId: 19870621,
    to: '',
    toId: -1,
    comments: [
      {
        name: '咸鱼不会飞',
        id: 19870621,
        headImg: '/src/assets/images/avatar/1.jpg',
        content:
          '真是一个紧迫的议题。现在全球竞争这么激烈，数字安全策略不仅关系到经济发展，更是国家安全的重要组成部分',
        time: '2024年3月20日 19:48',
        commentNum: 2,
        like: 15,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: 'Horizon',
            fromId: 19891221,
            fromHeadImg: '/src/assets/images/avatar/2.jpg',
            to: '咸鱼不会飞',
            toId: 19870621,
            content:
              '没错，看到华为在全球的挑战，真希望我们的数字安全可以得到更多重视',
            time: '2024年3月20日 20:43',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
          {
            from: '缘',
            fromId: 1123,
            fromHeadImg: '/src/assets/images/avatar/3.jpeg',
            to: '咸鱼不会飞',
            toId: 19870621,
            content: '这不仅是技术问题，更是战略布局问题，必须长远考虑',
            time: '2024年3月21日 21:56',
            commentNum: 0,
            like: 5,
            isLike: false,
            inputShow: false,
          },
        ],
      },
      {
        name: '飞翔的土豆',
        id: 19891221,
        headImg: '/src/assets/images/avatar/4.jpg',
        content: '中国在5G上的确走在了前列，希望能持续保持领先',
        time: '2024年3月26日 14:43',
        commentNum: 1,
        like: 5,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: 'Lana Del Rey',
            fromId: 19870621,
            fromHeadImg: '/src/assets/images/avatar/8.jpg',
            to: 'Taylor Swift',
            toId: 19891221,
            content: '关键还是要看实际应用和服务能不能跟上',
            time: '2024年3月26日  18:12',
            commentNum: 25,
            like: 5,
            isLike: false,
            inputShow: false,
          },
        ],
      },
      {
        name: '数字游侠',
        id: 20190830,
        headImg: '/src/assets/images/avatar/6.jpg',
        content:
          '看到这样的分析很振奋人心，希望我们的数字基础设施可以越来越强大',
        time: '2024年3月27日  11:43',
        commentNum: 0,
        like: 5,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: '源深',
            fromId: 19870621,
            fromHeadImg: '/src/assets/images/avatar/5.jpg',
            to: '数字游侠',
            toId: 19891221,
            content:
              '基础设施建设只是第一步，重要的是如何运用这些基础设施服务于经济和人民生活',
            time: '2024年3月30日 16:10',
            commentNum: 25,
            like: 5,
            isLike: false,
            inputShow: false,
          },
          {
            from: '漫步云端521',
            fromId: 19870621,
            fromHeadImg: '/src/assets/images/avatar/8.jpg',
            to: '数字游侠',
            toId: 19891221,
            content: '国家力量+民间智慧，双管齐下才能赢',
            time: '2024年3月30日 19:23',
            commentNum: 25,
            like: 5,
            isLike: false,
            inputShow: false,
          },
        ],
      },
      {
        name: 'Horizon',
        id: 20190830,
        headImg: '/src/assets/images/avatar/5.jpg',
        content: '数字安全太重要了，希望我们能有自己的核心技术',
        time: '2024年3月30日 18:33',
        commentNum: 0,
        like: 5,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: '飞翔的土豆',
            fromId: 19870621,
            fromHeadImg: '/src/assets/images/avatar/4.jpg',
            to: 'Horizon',
            toId: 19891221,
            content: '核心技术是保护伞，没有就会受制于人',
            time: '2024年3月30日 17:25',
            commentNum: 25,
            like: 5,
            isLike: false,
            inputShow: false,
          },
          {
            from: '咸鱼不会飞',
            fromId: 19870621,
            fromHeadImg: '/src/assets/images/avatar/6.jpg',
            to: 'Horizon',
            toId: 19891221,
            content: '自主研发才是王道，加油！',
            time: '2024年3月29日 19:23',
            commentNum: 25,
            like: 5,
            isLike: false,
            inputShow: false,
          },
        ],
      },
      {
        name: '极客少年',
        id: 20190830,
        headImg: '/src/assets/images/avatar/8.jpg',
        content: '华为的5G技术真是骄傲，希望能早日看到全国覆盖',
        time: '2024年3月30日 18:43',
        commentNum: 0,
        like: 5,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: '漫步云端521',
            fromId: 19870621,
            fromHeadImg: '/src/assets/images/avatar/7.jpg',
            to: '极客少年',
            toId: 19891221,
            content: '想象一下5G全覆盖的未来，太期待了！',
            time: '2024年3月29日 15:56',
            commentNum: 25,
            like: 5,
            isLike: false,
            inputShow: false,
          },
          {
            from: '数字游侠',
            fromId: 19870621,
            fromHeadImg: '/src/assets/images/avatar/6.jpg',
            to: '极客少年',
            toId: 19891221,
            content: '这将是一个巨大的进步，对整个社会都是',
            time: '2024年3月30日 06:23',
            commentNum: 25,
            like: 5,
            isLike: false,
            inputShow: false,
          },
        ],
      },
    ],
  },
  //
  {
    topicId: '1',
    btnShow: false,
    index: '0',
    replyComment: '',
    myName: '星辰大海007',
    myHeader: '/src/assets/images/avatar/2.jpg',
    myId: 19870621,
    to: '',
    toId: -1,
    comments: [
      {
        name: '漫步云端521',
        id: 19870621,
        headImg: '/src/assets/images/avatar/6.jpg',
        content:
          '人工智能的安全性确实不能忽视，看到Tay的故事后我觉得监管必不可少，但也不能限制了创新，这个平衡点挺难把握的',
        time: '2024年3月16日 18:43',
        commentNum: 2,
        like: 15,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: '飞翔的土豆',
            fromId: 19891221,
            fromHeadImg: '/src/assets/images/avatar/7.jpg',
            to: '漫步云端521',
            toId: 19870621,
            content: '说得对，既要发展又要安全，这责任不小啊',
            time: '2024年3月16日 19:23',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
          {
            from: '星辰大海007',
            fromId: 19891221,
            fromHeadImg: '/src/assets/images/avatar/6.jpg',
            to: '漫步云端521',
            toId: 19870621,
            content: '监管肯定需要，但是AI还是得自由发展，让它展现全部潜力',
            time: '2024年3月17日 17:43',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
        ],
      },

      {
        name: '码农小哥',
        id: 20190830,
        headImg: '/src/assets/images/avatar/7.jpg',
        content: '我们需要明确一点，AI是人创造的，所以出问题的不是AI，而是人',
        time: '2024年2月16日 12:43',
        commentNum: 0,
        like: 5,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: '茶余饭后233',
            fromId: 19891221,
            fromHeadImg: '/src/assets/images/avatar/8.jpg',
            to: '码农小哥',
            toId: 19870621,
            content: '确实，问题出在人身上，AI只是执行了错误的命令而已',
            time: '2024年2月18日 20:56',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
          {
            from: '清风徐来',
            fromId: 19891221,
            fromHeadImg: '/src/assets/images/avatar/2.jpg',
            to: '码农小哥',
            toId: 19870621,
            content: '问题是怎么防止人犯错呢？这是个系统性的大问题',
            time: '2024年2月18日 21:43',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
        ],
      },
      {
        name: '数字游侠',
        id: 20190830,
        headImg: '/src/assets/images/avatar/7.jpg',
        content: '这就是为什么AI的道德框架很重要，我们需要有道德的AI',
        time: '2024年2月18日 18:45',
        commentNum: 0,
        like: 5,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: '极客少年',
            fromId: 19891221,
            fromHeadImg: '/src/assets/images/avatar/1.jpg',
            to: '数字游侠',
            toId: 19870621,
            content: '道德框架听起来很科幻，但实施起来超难',
            time: '2024年2月19日 14:43',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
          {
            from: '梦里花落知多少',
            fromId: 19891221,
            fromHeadImg: '/src/assets/images/avatar/5.jpg',
            to: '数字游侠',
            toId: 19870621,
            content: '是啊，谁来定义“道德”呢？每个人的标准都不一样',
            time: '2024年2月19日 16:43',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
          {
            from: '极客少年',
            fromId: 19891221,
            fromHeadImg: '/src/assets/images/avatar/7.jpg',
            to: '数字游侠',
            toId: 19870621,
            content:
              '我更担心的是，如果AI学到了错误的东西，它可能会带来很大的危害',
            time: '2024年2月19日 15:21',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
          {
            from: '数字游侠',
            fromId: 19891221,
            fromHeadImg: '/src/assets/images/avatar/2.jpg',
            to: '极客少年',
            toId: 19870621,
            content: '确实，所以得有人时刻监控',
            time: '2024年2月20日 16:59',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
        ],
      },
    ],
  },
  {
    topicId: '2',
    btnShow: false,
    index: '0',
    replyComment: '',
    myName: 'Lana Del Rey',
    myHeader:
      'https://ae01.alicdn.com/kf/Hd60a3f7c06fd47ae85624badd32ce54dv.jpg',
    myId: 19870621,
    to: '',
    toId: -1,
    comments: [
      {
        name: 'Lana Del Rey',
        id: 19870621,
        headImg:
          'https://ae01.alicdn.com/kf/Hd60a3f7c06fd47ae85624badd32ce54dv.jpg',
        content: '我发布一张新专辑Norman Fucking Rockwell,大家快来听啊',
        time: '2019年9月16日 18:43',
        commentNum: 2,
        like: 15,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: 'Taylor Swift',
            fromId: 19891221,
            fromHeadImg:
              'https://ae01.alicdn.com/kf/H94c78935ffa64e7e977544d19ecebf06L.jpg',
            to: 'Lana Del Rey',
            toId: 19870621,
            content: '我很喜欢你的新专辑！！',
            time: '2019年9月16日 18:43',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
          {
            from: 'Ariana Grande',
            fromId: 1123,
            fromHeadImg:
              'https://ae01.alicdn.com/kf/Hf6c0b4a7428b4edf866a9fbab75568e6U.jpg',
            to: 'Lana Del Rey',
            toId: 19870621,
            content: '别忘记宣传我们的合作单曲啊',
            time: '2019年9月16日 18:43',
            commentNum: 0,
            like: 5,
            isLike: false,
            inputShow: false,
          },
        ],
      },
    ],
  },
  {
    topicId: '3',
    btnShow: false,
    index: '0',
    replyComment: '',
    myName: 'Lana Del Rey',
    myHeader:
      'https://ae01.alicdn.com/kf/Hd60a3f7c06fd47ae85624badd32ce54dv.jpg',
    myId: 19870621,
    to: '',
    toId: -1,
    comments: [
      {
        name: 'Lana Del Rey',
        id: 19870621,
        headImg:
          'https://ae01.alicdn.com/kf/Hd60a3f7c06fd47ae85624badd32ce54dv.jpg',
        content: '我发布一张新专辑Norman Fucking Rockwell,大家快来听啊',
        time: '2019年9月16日 18:43',
        commentNum: 2,
        like: 15,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: 'Taylor Swift',
            fromId: 19891221,
            fromHeadImg:
              'https://ae01.alicdn.com/kf/H94c78935ffa64e7e977544d19ecebf06L.jpg',
            to: 'Lana Del Rey',
            toId: 19870621,
            content: '我很喜欢你的新专辑！！',
            time: '2019年9月16日 18:43',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
          {
            from: 'Ariana Grande',
            fromId: 1123,
            fromHeadImg:
              'https://ae01.alicdn.com/kf/Hf6c0b4a7428b4edf866a9fbab75568e6U.jpg',
            to: 'Lana Del Rey',
            toId: 19870621,
            content: '别忘记宣传我们的合作单曲啊',
            time: '2019年9月16日 18:43',
            commentNum: 0,
            like: 5,
            isLike: false,
            inputShow: false,
          },
        ],
      },
    ],
  },
  {
    topicId: '4',
    btnShow: false,
    index: '0',
    replyComment: '',
    myName: 'Lana Del Rey',
    myHeader:
      'https://ae01.alicdn.com/kf/Hd60a3f7c06fd47ae85624badd32ce54dv.jpg',
    myId: 19870621,
    to: '',
    toId: -1,
    comments: [
      {
        name: 'Lana Del Rey',
        id: 19870621,
        headImg:
          'https://ae01.alicdn.com/kf/Hd60a3f7c06fd47ae85624badd32ce54dv.jpg',
        content: '我发布一张新专辑Norman Fucking Rockwell,大家快来听啊',
        time: '2019年9月16日 18:43',
        commentNum: 2,
        like: 15,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: 'Taylor Swift',
            fromId: 19891221,
            fromHeadImg:
              'https://ae01.alicdn.com/kf/H94c78935ffa64e7e977544d19ecebf06L.jpg',
            to: 'Lana Del Rey',
            toId: 19870621,
            content: '我很喜欢你的新专辑！！',
            time: '2019年9月16日 18:43',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
          {
            from: 'Ariana Grande',
            fromId: 1123,
            fromHeadImg:
              'https://ae01.alicdn.com/kf/Hf6c0b4a7428b4edf866a9fbab75568e6U.jpg',
            to: 'Lana Del Rey',
            toId: 19870621,
            content: '别忘记宣传我们的合作单曲啊',
            time: '2019年9月16日 18:43',
            commentNum: 0,
            like: 5,
            isLike: false,
            inputShow: false,
          },
        ],
      },
    ],
  },
  {
    topicId: '5',
    btnShow: false,
    index: '0',
    replyComment: '',
    myName: 'Lana Del Rey',
    myHeader:
      'https://ae01.alicdn.com/kf/Hd60a3f7c06fd47ae85624badd32ce54dv.jpg',
    myId: 19870621,
    to: '',
    toId: -1,
    comments: [
      {
        name: 'Lana Del Rey',
        id: 19870621,
        headImg:
          'https://ae01.alicdn.com/kf/Hd60a3f7c06fd47ae85624badd32ce54dv.jpg',
        content: '我发布一张新专辑Norman Fucking Rockwell,大家快来听啊',
        time: '2019年9月16日 18:43',
        commentNum: 2,
        like: 15,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: 'Taylor Swift',
            fromId: 19891221,
            fromHeadImg:
              'https://ae01.alicdn.com/kf/H94c78935ffa64e7e977544d19ecebf06L.jpg',
            to: 'Lana Del Rey',
            toId: 19870621,
            content: '我很喜欢你的新专辑！！',
            time: '2019年9月16日 18:43',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
          {
            from: 'Ariana Grande',
            fromId: 1123,
            fromHeadImg:
              'https://ae01.alicdn.com/kf/Hf6c0b4a7428b4edf866a9fbab75568e6U.jpg',
            to: 'Lana Del Rey',
            toId: 19870621,
            content: '别忘记宣传我们的合作单曲啊',
            time: '2019年9月16日 18:43',
            commentNum: 0,
            like: 5,
            isLike: false,
            inputShow: false,
          },
        ],
      },
    ],
  },
  {
    topicId: '5',
    btnShow: false,
    index: '0',
    replyComment: '',
    myName: 'Lana Del Rey',
    myHeader:
      'https://ae01.alicdn.com/kf/Hd60a3f7c06fd47ae85624badd32ce54dv.jpg',
    myId: 19870621,
    to: '',
    toId: -1,
    comments: [
      {
        name: 'Lana Del Rey',
        id: 19870621,
        headImg:
          'https://ae01.alicdn.com/kf/Hd60a3f7c06fd47ae85624badd32ce54dv.jpg',
        content: '我发布一张新专辑Norman Fucking Rockwell,大家快来听啊',
        time: '2019年9月16日 18:43',
        commentNum: 2,
        like: 15,
        isLike: false,
        inputShow: false,
        reply: [
          {
            from: 'Taylor Swift',
            fromId: 19891221,
            fromHeadImg:
              'https://ae01.alicdn.com/kf/H94c78935ffa64e7e977544d19ecebf06L.jpg',
            to: 'Lana Del Rey',
            toId: 19870621,
            content: '我很喜欢你的新专辑！！',
            time: '2019年9月16日 18:43',
            commentNum: 1,
            like: 15,
            isLike: false,
            inputShow: false,
          },
          {
            from: 'Ariana Grande',
            fromId: 1123,
            fromHeadImg:
              'https://ae01.alicdn.com/kf/Hf6c0b4a7428b4edf866a9fbab75568e6U.jpg',
            to: 'Lana Del Rey',
            toId: 19870621,
            content: '别忘记宣传我们的合作单曲啊',
            time: '2019年9月16日 18:43',
            commentNum: 0,
            like: 5,
            isLike: false,
            inputShow: false,
          },
        ],
      },
    ],
  },
])
export default commentList.value
