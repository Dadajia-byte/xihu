<template>
    <div class="list-box">
        <div class="type">
            <div class="type-item" v-for="(item, index) in type" :key="index" @click="filter(item)"
                :style="item === activityStore.filterItem.type ? 'background-color:#01a4ff;color:#fff;scale:1.2' : ''">{{
                    item }}
            </div>
        </div>
        ...
        <div class="list-card wow fadeInUp" v-for="(item, index) in activityStore.lists"
            ...
            <div class="card-left" :style="{
                ...
                <el-button class="dy" @click="sub(item.id)" v-show="item.isSub == 0">订阅</el-button>
                <el-button class="dy" @click="cancelSub(item.id)" v-show="item.isSub != 0">取消订阅</el-button>
                <div class="c-c">
                    {{ item.content }}
                </div>
                <div class="c-t">
...
let type = ['全部', '格致论道', '西湖论剑安全特训营', '新品发布', '第二直播间', '掌上论剑', '西湖论剑十周年回顾']
...
.list-box {
    ...
    border-radius: .3rem;
    box-shadow: 0 0 .125rem 0px rgb(228, 228, 228);
    padding: 0 .8333rem .8667rem .8333rem;
    ...
    .type {
        ...
        &::after {
            ...
            height: .0313rem;
            /* 设置边框的高度 */
            background: linear-gradient(to right, #004dbd, #014fbb, #019bc4, #1bc2c1, #aeebe4, #ffffff);
        }
