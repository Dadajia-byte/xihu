<template>
    <div>
        <el-card class="box-card">
            <el-form label-width="150px" ref="form" style="width: 7rem;" :model="userStore.userData">
                <el-upload class="avatar-uploader" action="/dev-api/upload" :show-file-list="false"
                    :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload"
                    style="margin-left: 3.125rem;margin-bottom: .25rem;" :disabled="!isEdit">
                ...
                <el-form-item label="账号:">
                    ...
                    <div class="gray" v-show="isEdit">{{ userStore.userData.account }}</div>
                </el-form-item>
                ...
                <el-form-item label="生日:">
                    <el-date-picker v-model="userStore.userData.birth" type="date" placeholder="Pick a date"
                        style="width: 100%" :disabled="!isEdit" />
                </el-form-item>
                <el-form-item label="职业">
                    <div v-show="!isEdit">{{ userStore.userData.position || '暂无信息' }}</div>
                ...
                <el-form-item label="工作单位/学校:">
                    <div v-show="!isEdit">{{ userStore.userData.company || '暂无信息' }}</div>
                    <el-input v-if="isEdit" v-model="userStore.userData.company" placeholder="请填写工作单位/学校"></el-input>
                </el-form-item>
                <el-form-item label="邮箱:">
                    <div v-show="!isEdit">{{ userStore.userData.email || '暂无信息' }}</div>
                    <el-input v-model="userStore.userData.email" v-if="isEdit" placeholder="请填写你的邮箱"></el-input>
                </el-form-item>
            </el-form>
            <el-button v-show="!isEdit" @click="isEdit = true" style="margin-left: 3.75rem;">更改信息</el-button>
            <el-button v-show="isEdit" style="margin-left: 3.75rem;" @click="cancel">取消</el-button>
...
const save = async () => {
    isEdit.value = false
    await userStore.updateUser().then(() => {
...
.avatar-uploader .el-upload {
    border: .0125rem dashed var(--el-border-color);
