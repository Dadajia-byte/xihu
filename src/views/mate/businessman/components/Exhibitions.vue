<template>
    <div class="exhibitions-box">
        ...
        <div class="outline">
            ...
            @click="handleGoAr"
                :style="{ animationDelay: (index + 1) * 0.3 + 's' }" v-for="(item, index) in exhibitors">
                <img :src="item" alt="">
            </div>
        </div>
        <div class="inline">
            <div class="e" v-for="(item, index) in inlineE" :key="item" :class="'e_' + (index + 1)"
            @click="handleGoWeb"
...
const handleGoWeb = () => {
    window.open('https://ve.mvpmeta.com/new/preview/#/?id=16159&secret=MTYxNTk=&ratio=256')
}
...
.exhibitions-box {
    ...
    .outline {
        ...
        transform: translate(-50%, -50%);
        z-index: 1000;
        border: .0333rem solid $theme-color-green;
        border-radius: 50%;
        ...
        .e {
            ...
            border-radius: 50%;
            position: absolute;
            border: .0167rem solid rgba(240, 240, 240, 0.655);
            display: flex;
            justify-content: center;
            align-items: center;
            background-color: white;
            box-shadow: 0 0 .1667rem 0px rgba(197, 197, 197, 0.1);
    ...
    .inline {
        ...
        transform: translate(-50%, -50%);
        z-index: -1;
        border: .0333rem solid rgb(177, 192, 202);
        border-radius: 50%;
        z-index: 1000;
    ...
    .line {
        ...
        left: 0;
        top: 0;
        clip-path: polygon(calc(100% - .5rem) 0%, 0% calc(100% - .5rem), .5rem 100%, 100% .5rem);
    ...
    .cir-r {
        top: 0;
        right: 0;
        background: radial-gradient($theme-color-green .2667rem, $theme-color-blue );
    ...
    .cir-l {
        bottom: 0;
        left: 0;
        background: radial-gradient($theme-color-blue .4667rem, $theme-color-green);
